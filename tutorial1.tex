\documentclass[a4paper, 11pt]{article} %Here you can change the font size of the entire document

%\usepackage{showframe} %Helper package to show document outlines

\usepackage{sectsty} %Package needed to format the section headings. This package is responsible for the \underbar and \underline errors
\usepackage{graphicx} %Package for importing graphics (pics)
\usepackage{ragged2e} %Package for justifying text
\usepackage{tikz} %Package for advanced image positioning
\usepackage{xcolor} %Package for changing the color of text
\usepackage{etoolbox} %Package to change abstract title to be at the top of the page
\patchcmd{\abstract}{\null\vfil}{}{}{} %Abstract patch: do nog delete
\usepackage{tocloft} %Package to properly format Table of Contents
\setlength{\cftbeforetoctitleskip}{0pt}
\setlength{\cftaftertoctitleskip}{0pt}
\usepackage[a4paper, total={16cm, 24cm}]{geometry} %Specifies the size of text area on the paper
\usepackage{hyperref} %Package to make the Table of Contents use hyperlinks
\usepackage{natbib} %Package to use Harvard style bibliography, comment out if using IEEE style
\usepackage{helvet} %Arial Font
\renewcommand{\familydefault}{\sfdefault} %Command needed to make font arial

%Formatting of headings
\sectionfont{\LARGE \color{Puk_Pers}}
\subsectionfont{\Large \color{Puk_Pers}}
\subsubsectionfont{\large \color{Puk_Pers}}
\renewcommand\abstractname{\LARGE \textcolor{Puk_Pers}{Abstract}} %To make the abstract purple
\renewcommand{\contentsname}{\center\bfseries\Large \textcolor{Puk_Pers}{Table of Contents}\center}
\renewcommand{\listfigurename}{\center\bfseries\Large \textcolor{Puk_Pers}{List of Figures}\center}
\renewcommand{\listtablename}{\center\bfseries\Large \textcolor{Puk_Pers}{List of Tables}\center}      
\renewcommand\thesection{\arabic{section}} %Specifies the numbering of sections
%\renewcommand{\refname}{Bibliography}


%Page formatting
\setlength{\parindent}{0in} 
\setlength{\parskip}{8pt} %The space between paragraphs
\renewcommand{\baselinestretch}{1.08} %The line spacing

\graphicspath{ {cover/} } %Directory for cover image

\definecolor{Puk_Pers}{RGB}{108, 61, 145}

\begin{document}
\begin{titlepage}

\begin{tikzpicture}[remember picture, overlay, transform shape]
\node [anchor=north west, inner sep=0pt]
    at (current page.north west)
    {\includegraphics[width=\paperwidth]{NWU_Purple_Cover.png}};
\end{tikzpicture}	
	\centering

	\vspace{4cm}
	{\huge\bfseries \textcolor{Puk_Pers}{PROJECT TITLE}\par}
	\vspace{2cm}
	{\Large\itshape \textcolor{Puk_Pers}{INITIALS AND SURNAME}\par}
	\vspace{2cm}
		{\scshape\LARGE DOCUMENT TYPE HERE  \par}
	\vspace{5cm}
\begin{flushleft}
	Supervisor: SUPERVISOR NAME\par
	Date: \today  \par
	Student No: STUDENT NO\par
\end{flushleft}

\end{titlepage}

\thispagestyle{plain}

\begin{abstract}
\addcontentsline{toc}{section}{Abstract} %Abstract needs to be added to table of contents
\pagenumbering{roman}
\justify


WRITE YOUR ABSTRACT HERE

\end{abstract}
\newpage

\tableofcontents

\listoffigures

\listoftables

\pagebreak

\pagenumbering{arabic}

\section{Hoofstuk 1}
Bla bla bla 
\subsection{Subsection 1}
bla bla
\subsubsection{SubSubsection 1}

\subsection{Subsection 2}
bla bla

\section{Section 2}
Bla bla bla \cite{CountdownTimer}


%Picture tutorial
\begin{figure}[h]
\center
\includegraphics[scale=0.5]{NWU_Purple_Cover.png}
\textcolor{Puk_Pers}{\caption{CAPTION}}
\label{LABEL_NAME}
\end{figure}

With refernce to figure \ref{LABEL_NAME} 


%Equation
\begin{equation} 
x^2 + 4x + 5 
\label{EQUATION NAME}
\end{equation}

%Tables
%To generate tables make use of https://tablesgenerator.com/
%Add caption, label and \center to the table code just underneath the \begin{table}
%e.g.
\begin{table}[]
\center
\textcolor{Puk_Pers}{\caption{CAPTION}}
\vspace{0.1cm}
\label{LABEL NAME}
\begin{tabular}{lll}
la & la   \\
la & la   
\end{tabular}
\end{table}

\citep{CountdownTimer}


\addcontentsline{toc}{section}{References}
%\bibliographystyle{IEEEtran} %- IEEE Style
\bibliographystyle{humannat} %- Harvard style
\bibliography{references}
\end{document}
